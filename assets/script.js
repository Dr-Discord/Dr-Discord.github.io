HTMLElement.prototype.oneTimeEventListener = function(event, callback, options) {
  function oneTime() {
    callback(...arguments)
    this.removeEventListener(event, oneTime, options)
  }
  this.addEventListener(event, oneTime, options)
}
window.oneTimeEventListener = HTMLElement.prototype.oneTimeEventListener
...
function zoom(onWheelElement, zoomElement = onWheelElement) {
  ...
  onWheelElement.onwheel = (event) => {
    ...
    zoomElement.style.transition = "transform .2s"
  }
}
function refreshCSS() {
...
function nextTick(callback, ...args) { return setTimeout(() => callback(...args), 4) }
document.addEventListener("DOMContentLoaded", () => {
  ...
  function makeContextMenu(Element, menuItems) {
    ...
    if (!menuItems) return console.error("menuItems is null or isnt a array")
    if (!Array.isArray(menuItems)) menuItems = [menuItems]
    function makeMenu(e) {
      e.preventDefault()
      remove()
      let validMenuItems = menuItems.filter(e => typeof e.showIf === "function" ? e.showIf() : true)
      if (!validMenuItems.length) return
      menu = document.createElement("div")
      Object.assign(menu.style, {
        top: `${e.clientY}px`,
        left: `${((innerWidth - 200) <= e.clientX) ? e.clientX - 200 : e.clientX}px`,
        position: "fixed",
        zIndex: "999"
      })
      ...
      oneTimeEventListener("click", remove)
    }
    Element.oncontextmenu = (e) => isMobile() ? null : makeMenu(e)
    Element.ontouchstart = (e) => {
      const event = e.touches[0]
      let isTouching = true
      setTimeout(() => isTouching ? makeMenu(Object.assign(event, { preventDefault: () => e.preventDefault() })) : null, 500)
      Element.ontouchend = () => {
        isTouching = false
        return true
      }
  ...
  let outMost
  document.querySelector(".discord_icon").onclick = () => {
  ...
  for (const img of document.getElementsByTagName("img")) {
    ...
    img.onclick = () => {
      ...
      makeContextMenu(clone, [{
        type: "item",
        name: "Copy Image URL",
        action: () => ToClipboard(clone.src)
      }])
